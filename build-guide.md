...
torabo-tsuki LPシリーズのビルドガイドです。V1とV2では電池ボックスの取り付け周りの手順が異なります。
自分が組み立てるのはどちらかを確認してから手順を進めてください。
...
| 名称                                           | 数    | 補足                                                                                                                                   |
| ---------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------- |
| キースイッチ(Choc V2)                          | 42(S, XS), 50(M), 64(L) |                                                                                                                                        |
| キーキャップ                                   | 適量  |                                                                                                                                        |
...
| 乾電池                                       | 2     | S,M,Lは単三、XSは単四、Ni-MH可。USBで充電できるリチウム電池内蔵のものを使う場合、必ずキーボードから取り外してから充電してください                             |
| **オプション** |||
...
電池カバーは、S,M,Lの場合はaaを、XSの場合はaaaを使用してください。V2ではV1から形状が若干変わっています。過去にV1を組み立て済みで、今回V2を組み立てる方は、改めてデータをダウンロードしてください。

また、[シンプルケース](3d-models/STL/option)、[テンティングケース](https://github.com/sekigon-gonnoc/torabo-tsuki-lp-tenting-case)や、有志が公開・販売しているデータを一緒に製造することもできます。ご自分でデザインしたい場合は[シンプルケースのCADデータ](3d-models/FreeCAD/option/torabo-tsuki-lp-simple-case.FCStd)をご活用ください。
...
ニッパーでそのまま切り離せない場合は、表裏に切れ目を入れてから折り取ることもできますが、ほかの配線を傷つけないように注意してください。
...
### (V1の場合)電池ボックスをはんだ付けする

* 電池ボックスの足を折り曲げます。
  * XSに単四電池ケースを取り付ける場合は基板上のマークよりすこし上に寄せてください。基板の穴はBMP Boost側を使います。マイナス極が上側です。
...
|![](img/aaa-battery-position.JPG)|
|-|
|単四電池ケースの取り付け位置|

### (V2の場合)電池ボックスを基板に差し込む

torabo-tsuki LP V2では電池ボックスもソケットに差し込むだけで取り付けられます。
ソケットの位置に合わせて電池ボックスの足を折り曲げてから差し込みます。
S,M,Lではパーツ配置の都合で斜め方向に足を曲げ、XSでは真上に曲げてください。

|![](img/v2-aa-side.jpg)|![](img/v2-aaa-side.jpg)|
|-|-|
|S,M,L|XS|

* 電池ボックスのピンを折り曲げます。横から見たときピンが真上に向くように折り曲げていきます。手で90度まで曲げても手を離すと手前まで戻ってしまうので、以下のような手順で進めてください。
  ![](img/v2-box-pin.jpg)
  1. 電池ボックスのピン先端をつかみ、真横を向くように曲げます。手を離すと下に少し戻るので、様子を見ながら少しずつ曲げ量を増やします。S,M,Lではパーツの配置の都合上、下から見たとき少し斜めになるようにしてください。XSは真っすぐで大丈夫です。

      |![](img/v2-box-step1.jpg)|
      |-|
      |横から見たとき|
      |![](img/v2-box-aa-step1.jpg)|
      |S,M,L|
      |![](img/v2-box-step1.1.jpg)|
      |XS|

  2. ボトムプレートを間に挟みながらもう90度曲げます。手を離すと90度より手前に戻りますが、そのまま次に進みます。

      |![](img/v2-box-step2.jpg)|![](img/v2-box-step3.jpg)|
      |-|-|

  3. 電池ボックスのピン先端をつかんでさらに曲げます。電池ボックスと干渉するので、このステップでも90度までは曲がりません。

      ![](img/v2-box-step4.jpg)

  4. 電池ボックスの横に空いている穴の上端のところで、電池ボックスのピンをニッパーでカットします。

      ![](img/v2-box-step5.jpg)

  5. ピンの先端を電池ボックスの横の穴に押し込むように曲げます。手を離したときにピンが真上を向くところまで曲げてください。

      ![](img/v2-box-step6.jpg)

* 電池ボックスをメイン基板に差し込む
  * 電池ボックスのピンが、基板の板バネでしっかり挟みこまれていることを確認してください

|![](img/v2-box-plus.jpg)|![](img/v2-box-minus.jpg)|
|-|-|
...
### (V2の場合)電池ボックスの余った足をカットする

* 電池ボックスをボトムプレートに接地するように下側に寄せて、余った足をニッパーでカットしてください。
  * 電池ボックスがボトムプレートから浮いた状態でカットしてしまうと、使用中に電池ボックスが下側に抜けてしまう可能性があるので、注意してください。

|![](img/v2-box-plus-cut.jpg)|![](img/v2-box-minus-cut.jpg)|
|-|-|
...
> [!caution]
> V2基板に対してV1用のカバーを無理やり取り付けようとした場合、電池ボックス用のソケットが破損します。
> V2用カバーを取り付ける場合も基板側面にカバーを確実に押し当てなが内側の爪をひっかけるようにしてください。
>
> ![](img/v2-battery-caution.jpg)
...
* Keymap-Editorを使用する場合は[zmk-keyboard-torabo-tsuki-lp](https://github.com/sekigon-gonnoc/zmk-keyboard-torabo-tsuki-lp)をクローンして編集してください。
  ...
  * [ファームウェアの書き込み方](#ファームウェアを書きこむ)を参考にして、新しく作ったuf2ファイルを書き込んでください。
...
### ペリフェラル側をUSB接続してもキー入力できない

* ZMKの仕様です
* セントラルとペアリングしたうえでセントラル側を有線または無線接続してください
